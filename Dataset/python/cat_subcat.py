...
for each_product in json_data['response']['docs']:
    ...
    # try:
    #     if each_product.get('category', '') not in category_dict:
    #         category_dict[each_product.get('category', '')] = gs.translate(each_product.get('category', ''), 'en', 'th')
    # except (urllib2.HTTPError, urllib2.URLError, KeyError):
    #     category_dict[each_product.get('category', '')] = ''

    # try:
    #     if each_product.get('subcategory', '') not in subcategory_dict:
    #         subcategory_dict[each_product.get('subcategory', '')] = gs.translate(each_product.get('subcategory', ''), 'en', 'th')
    # except (urllib2.HTTPError, urllib2.URLError, KeyError):
    #     subcategory_dict[each_product.get('subcategory', '')] = ''
...
# print len(category_dict), len(subcategory_dict)
# print len(title)

# for word in category_dict:
#     print word, category_dict[word]

final_list['category_mapping'] = category_dict
final_list['subcategory_mapping'] = subcategory_dict
write_file = open('lazada_output.json', 'w')
write_file.write(json.dumps(final_list, indent=4))
