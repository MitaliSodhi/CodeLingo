...
//  Solutions to exercises 12-6 and 12-7.
...
/* Class TextWindow has been modified to solve the 
   problem presented in Exercise 12-7.  */

class  TextWindow  :  FrameWindow
{
   protected void embed_text_in_window( string text_inside_window )
   {
   ...
   public TextWindow( int     desired_window_width,
      ...
      :  base( desired_window_width, desired_window_height )
   {
      embed_text_in_window( given_line_of_text ) ;
   }

   public void change_text( string  new_line_of_text )
   {
      embed_text_in_window( new_line_of_text ) ;
   }
}
...
//  The following class is the solution to Exercise 12-6.

class ExtraDecoratedTextWindow  :  DecoratedTextWindow
{
   public ExtraDecoratedTextWindow( int    desired_window_width,
                                    int    desired_window_height,
                                    string given_line_of_text ) 
      ...
      :  base( desired_window_width,
               ...
               given_line_of_text )
   {
      Window extra_decoration_frame  =  new  Window( desired_window_width,
                                                     desired_window_height,
                                                     '*' ) ;
      DecoratedTextWindow  window_inside_window  = 
                  new  DecoratedTextWindow( desired_window_width - 2,
                                            desired_window_height - 2,
                                            given_line_of_text ) ;

      extra_decoration_frame.move( 1, 1, window_inside_window ) ;

      move( 0, 0, extra_decoration_frame ) ;
   }
}


class WindowsSolutions
{
   static void Main()
   {
      ...
      //  similing_window is now of type ExtraDecoratedTextWindow

      ExtraDecoratedTextWindow  smiling_window  =
                       new  ExtraDecoratedTextWindow( 28, 11, "Smile!" ) ;
