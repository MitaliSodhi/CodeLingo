...
//  CollectSortable.cs  (c) 2004 Kari Laitinen
...
//  A solution to Exercise 15-7.
//  This version is made from CollectWithArrayList.cs.
...
class CollectionItem : IComparable
{
   string  item_name ;
   string  item_maker ;
   string  item_type ;
   ...
   {
      CollectionItem  item_to_compare_to  =
                          (CollectionItem) object_to_compare_to ;

      return  String.Compare( item_maker,
                              item_to_compare_to.item_maker ) ;
...
class  CollectionMaintainer
{
   ...
   public void run()
   {
      ...
      while  (  user_selection[ 0 ]  !=  'q' )
      {
         ...
         else if ( user_selection[ 0 ]  ==  'o' )
         {
            this_collection.sort_collection() ;
         }
